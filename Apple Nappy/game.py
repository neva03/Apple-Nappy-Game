...
pygame.display.set_caption('Apple Nappy')

#Sounds
countdown_sound = pygame.mixer.Sound("sounds/countdown.mp3")
gameover_sound = pygame.mixer.Sound("sounds/gameover.mp3")
skull_sound = pygame.mixer.Sound("sounds/skullsound.mp3")
collect_sound = pygame.mixer.Sound("sounds/collect.mp3")

#Fonts
main_font = 'fonts/smallest_pixel-7.ttf'
title_font = 'fonts/BACKTO1982.ttf'
cd_font ='fonts/Kamikaze3DGradient.ttf'
...
apple_image = pygame.transform.scale(apple_image, (40, 50))
goldenapple_image = pygame.image.load("sprites/goldenapple.png")
goldenapple_image = pygame.transform.scale(goldenapple_image, (40, 50))

# Skull image
skull_image = pygame.image.load("sprites/skull.png")
skull_image = pygame.transform.scale(skull_image, (60, 60))

# Background image
background_image = pygame.image.load("sprites/background.gif")
background_image = pygame.transform.scale(background_image, (screenWidth, screenHeight))

# Cart image
cart_image = pygame.image.load(r"sprites/cart.png")
cart_image = pygame.transform.scale(cart_image, (100, 80))

#function to draw the cart 
def draw_plate():
     plate_y =+510
     gameDisplay.blit(cart_image, (plate_x, plate_y))
...
def draw_pause_button():
    ...
    button_y = 60
    draw_button("Pause", button_x, button_y, button_width, button_height, pink, black, pause_game)
...
def draw_button(text, x, y, width, height, inactive_color, active_color, action=None):
    ...
    # Draw button text
    button_font = pygame.font.Font(main_font, 36)
...
def restart_game():
    global is_failed, score, circles, plate_x, lives, gameover_sound_played
    is_failed = False
    score = 0
    lives = 3
    circles = []
...
def game_over():
    global max_score, gameover_sound_played
    ...
    font = pygame.font.Font(main_font, 55)
    ...
    draw_button("Restart", button_x, button_y, button_width, button_height, pink, black, restart_game)
...
def countdown():
    ...
    for i in range(3, 0, -1):  # 3'ten 1'e kadar sayar
        for size in range(50, 150, 5):  # Yazı boyutunu büyütür (50'den 150'ye)
            ...
            countdown_font = pygame.font.Font(cd_font, size)
...
def show_menu():
    ...
    while menu_running:
        ...
        # Display game title
        font = pygame.font.Font(title_font, 67)
        title_surface = font.render("Apple Nappy", True, black)
        ...
        draw_button("Start Game", button_x, start_button_y, button_width, button_height, pink, black, start_game)
        
        # Show "Resume" button only if the game is paused
        if is_paused:
            draw_button("Resume", button_x, resume_button_y, button_width, button_height, pink, black, resume_game)
        
        draw_button("Quit", button_x, quit_button_y, button_width, button_height, pink, black, quit_game)
...
def pause_game():
    ...
    while is_paused:
        ...
        # Display "Paused" text
        font = pygame.font.Font(main_font, 70)
        ...
        draw_button("Resume", button_x, resume_button_y, button_width, button_height, pink, black, resume_game)
        draw_button("Go to Menu", button_x, menu_button_y, button_width, button_height, pink, black, show_menu)
...
while running:
    ...
    if not is_failed and not is_paused:  # Update game state if not paused or game over
        ...
        if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and plate_x > 0:
            plate_x -= plate_speed
        if (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and plate_x < screenWidth - plate_width:
            plate_x += plate_speed
